...
describe('IFO contract', function() {
  let alice, bob, carol, dev, minter;
  let lp, ifoToken;
  let Token, IFO;

  beforeEach(async function() {
    Token = await ethers.getContractFactory("MockBEP20");
    IFO = await ethers.getContractFactory("IFO");
    
    [alice, bob, carol, dev, minter] = await ethers.getSigners()

    lp = await Token.deploy()
    await lp.deployed()

    ifoToken = await Token.deploy()
    await ifo.deployed()

  })

  it('raise not enough lp', async function () {
    this.ifo = await IFO.deploy(this.lp.address, ifoToken.address, '20', '30', '100', '10', alice);
